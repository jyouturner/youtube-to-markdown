...
**Phase 3 — subscription scheduler migration to Cowork.** Adds the `yt2md-poll-subscriptions` skill + two MCP tools (`list_new_channel_videos`, `mark_channel_video_seen`) so the user can wire Cowork's Desktop Scheduled Tasks to drive autonomous polling — replacing yt2md's in-process scheduler (`_scheduler_loop` in `cmd_serve`). `list_new_channel_videos` preserves the existing no-backfill-on-first-poll semantic: seeds `~/yt2md/state.json` on first call and returns `[]`; subsequent calls return videos whose ID is neither in the seen cursor nor on disk as a digest dir. `mark_channel_video_seen` is the post-success hook. The old `_scheduler_loop` and `cmd_watch_run` paths remain functional during the transition — the user opts into the Cowork-driven flow by creating a Desktop Scheduled Task pointing at `/yt2md-poll-subscriptions`; the in-process scheduler can be disabled via the Schedule page when the Cowork path is verified working. Phase 4 will delete the legacy scheduler + the three LLM backends once everyone's switched over.
